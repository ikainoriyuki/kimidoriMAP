...
function createPopupContent(poi) {
  ...
  return `<div class="popup-content">時間: ${poi.timestamp.toLocaleDateString()}_${poi.timestamp.toLocaleTimeString()}<br>測位: ${poi.positioning}<br>番号: ${poiNumber}<br>属性: ${poiDescription}</div>` +
    `<br><button id='edit-btn' class='edit-button'>編集</button> <button id='delete-btn' class='delete-button'>削除</button>`;
}

// --- POI登録/編集/削除 ---

async function openPOIModal(title, defaultNumber, defaultDescription) {
  return new Promise((resolve) => {
    ...
    newSaveBtn.addEventListener('click', function saveHandler() {
      ...
      cleanup();
      resolve({ number: newNumber, description: newDescription });
    }, { once: true }); // 一度だけ実行
...
async function saveCurrentLocation(map, uiControls) {
  ...
  const result = await openPOIModal("現在地を登録", suggestedNumber, ""); 
  
  if (result) {
    let newPOI = {
      latlng: markerLatLng, timestamp: new Date(), positioning: "Positioning-based",
      number: result.number, description: result.description
    };
...
async function saveMapCenterLocation(map, uiControls) { 
  ...
  const suggestedNumber = getPreviousPOINumber() + 1;
  const result = await openPOIModal("画面中央を登録", suggestedNumber, "");
...
async function editPOI(poi, marker) {
  const result = await openPOIModal("地点を編集", poi.number, poi.description);
