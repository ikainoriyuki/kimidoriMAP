...
// ============================================================
// 完成済みポリゴンの頂点編集（移動・削除・挿入）
// ============================================================

let _ctx             = null;
let _editingRec      = null;
let _polygonLayer    = null;   // L.Polygon（L.GeoJSON の内側レイヤー）
let _originalLatlngs = null;   // cancel 用スナップショット
let _originalFeature = null;   // cancel 用スナップショット
let _vertexMarkers   = [];
let _midpointMarkers = [];
let _selectedIndex   = null;
let _insertionIndex  = null;
...
export function initPolygonVertexEditModule(ctx) {
  _ctx = ctx;
  document.getElementById('vertex-edit-commit-btn')
    ?.addEventListener('click', () => { if (_isEditing) commitPolygonVertexEdit(); });
  document.getElementById('vertex-edit-cancel-btn')
    ?.addEventListener('click', () => { if (_isEditing) cancelPolygonVertexEdit(); });
  document.getElementById('vertex-edit-move-btn')
    ?.addEventListener('click', () => { if (_isEditing) _setEditMode('move'); });
  document.getElementById('vertex-edit-delete-btn')
    ?.addEventListener('click', () => { if (_isEditing) _setEditMode('delete'); });
  document.getElementById('vertex-edit-insert-btn')
    ?.addEventListener('click', () => { if (_isEditing) _setEditMode('insert'); });
}

export function isPolygonVertexEditActive() { return _isEditing; }

export function startPolygonVertexEdit(map, rec) {
  if (_ctx.isSplitModeActive?.()) { showToast('分割モード中は頂点編集を開始できません。'); return; }
  if (_ctx.lineMode.active) {
    alert('ライン記録中は頂点編集できません。');
    return;
  }
  if (_ctx.isLineVertexEditActive?.()) {
    alert('ライン頂点編集中はポリゴン頂点編集を開始できません。');
    return;
  }
  if (_isEditing) cancelPolygonVertexEdit();

  _map           = map;
  _editingRec    = rec;
  _polygonLayer  = rec.layer.getLayers()[0];
  _originalLatlngs = _polygonLayer.getLatLngs()[0].map(ll => L.latLng(ll.lat, ll.lng));
  _originalFeature = JSON.parse(JSON.stringify(rec.feature));
  _editMode      = 'move';
  _selectedIndex = null;
...
function _buildVertexMarkers() {
  ...
  _polygonLayer.getLatLngs()[0].forEach((ll, i) => {
    const m = L.marker(ll, {
...
// ポリゴンは閉じたリングなので n 頂点に対し n 中間点
function _buildMidpointMarkers() {
  _removeMidpointMarkers();
  const latlngs = _polygonLayer.getLatLngs()[0];
  const n = latlngs.length;
  for (let i = 0; i < n; i++) {
    const next = latlngs[(i + 1) % n];
    const mid = L.latLng(
      (latlngs[i].lat + next.lat) / 2,
      (latlngs[i].lng + next.lng) / 2,
    );
    const idx = i;
...
function _deleteVertex(index) {
  const latlngs = _polygonLayer.getLatLngs()[0];
  if (latlngs.length <= 3) {
    alert('ポリゴンの頂点は最低3点必要です。');
    return;
  }
  latlngs.splice(index, 1);
  _polygonLayer.setLatLngs([latlngs]);
  _buildVertexMarkers();
  _selectedIndex = null;
}

// ============================================================
// POIクリックへの割り込み（移動・挿入モード）
// ============================================================
...
async function commitPolygonVertexEdit() {
  if (!_editingRec) return;
  _unpatchPOIClickHandlers();
  _removeVertexMarkers();
  _removeMidpointMarkers();

  const updated = _polygonLayer.toGeoJSON();
  _editingRec.feature = { ..._editingRec.feature, geometry: updated.geometry };

  await _ctx.updatePolygonInDB({
    id:          _editingRec.id,
    feature:     _editingRec.feature,
    timestamp:   _editingRec.timestamp,
    name:        _editingRec.name,
    description: _editingRec.description,
  });
  _ctx.refreshPolygonPopup(_editingRec);
  _ctx.updatePolygonLabel(_editingRec);
...
function cancelPolygonVertexEdit() {
  ...
  _polygonLayer.setLatLngs([_originalLatlngs]);
  _editingRec.feature = _originalFeature;
...
function _reset() {
  _editingRec = null; _polygonLayer = null;
  _originalLatlngs = null; _originalFeature = null;
  _selectedIndex = null; _insertionIndex = null;
  _isEditing = false; _map = null;
}
